...
/**
 * Tests for GangliaClusterHostsLoader.
 *
 * @author Gil Markham (gil at groupon dot com)
 */
class GangliaClusterHostsLoaderTest {
    ...
    val configCaptor = captor<GangliaClusterHosts>()
    val loader = GangliaClusterHostsLoader(vertx, File("/tmp"))
    ...
    fun testReadExampleFile() {
        ...
        Mockito.verify(localMap).put(Matchers.eq(GangliaClusterHostsLoader.NAME), configCaptor.capture())
        val config = configCaptor.value
        assertEquals(setOf("ganglia_host12", "ganglia_host14"), config.getHosts("cluster15"))
    }
}
